...
class CherryAI_v8_UniversalIntelligentOrchestrator(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        self.streaming_updater = TaskUpdater(event_queue, context.task_id, context.context_id)
        
        try:
            user_input = self._extract_user_input(context)
            if not user_input:
                await self.streaming_updater.update_status(
                    TaskState.failed,
                    message=self.streaming_updater.new_agent_message(parts=[TextPart(text="❌ 사용자 입력을 찾을 수 없습니다.")])
                )
                return
            ...
            await self.streaming_updater.update_status(
                TaskState.working,
                message=self.streaming_updater.new_agent_message(parts=[TextPart(text="🔍 A2A 프로토콜로 에이전트를 발견하고 있습니다...")])
            )
            ...
            await self.streaming_updater.update_status(
                TaskState.working,
                message=self.streaming_updater.new_agent_message(parts=[TextPart(text="🧠 요청 복잡도를 분석하고 있습니다...")])
            )
            ...
            await self.streaming_updater.update_status(
                TaskState.working,
                message=self.streaming_updater.new_agent_message(parts=[TextPart(text=f"📊 요청 복잡도: {complexity_level}")])
            )
            ...
            await self.streaming_updater.update_status(
                TaskState.failed,
                message=self.streaming_updater.new_agent_message(parts=[TextPart(text=f"❌ 처리 중 오류가 발생했습니다: {str(e)}")])
            )
...
if __name__ == "__main__":
    ...
    print("🚀 Starting CherryAI v8 Universal Intelligent Orchestrator")
    print("🌐 Server starting on http://localhost:8100")
    print("📋 Agent card: http://localhost:8100/.well-known/agent.json")
    print("🧠 Features: A2A Discovery + Real-time Streaming + LLM Intelligence")
    
    uvicorn.run(app.build(), host="0.0.0.0", port=8100, log_level="info") 