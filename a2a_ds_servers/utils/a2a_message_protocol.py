#!/usr/bin/env python3
"""
A2A 메시지 프로토콜 통일화 유틸리티

A2A SDK 0.2.9 표준을 준수하는 메시지 생성, 파싱, 검증 함수들을 제공합니다.
모든 A2A 에이전트에서 일관된 메시지 처리를 위한 표준 인터페이스입니다.
"""
...
class A2AMessageProtocol:
    """A2A SDK 0.2.9 표준 메시지 프로토콜 통일화 클래스"""
    ...
    @staticmethod
    def create_send_request(message: Message, request_id: Optional[str] = None) -> SendMessageRequest:
        """
        표준 SendMessageRequest 생성
        
        Args:
            message: 전송할 Message 객체
            request_id: 요청 ID (생략 시 자동 생성)
            
        Returns:
            SendMessageRequest: A2A 표준 요청 객체
        """
        if not request_id:
    ...
    def extract_user_input(context: RequestContext) -> str:
        """
        RequestContext에서 사용자 입력 추출 (표준 방식)
    ...
    def validate_message(message: Message) -> tuple[bool, List[str]]:
        ...
        if not hasattr(message, 'parts') or not message.parts:
            ...
            for i, part in enumerate(message.parts):
                if not hasattr(part, 'text') and not (hasattr(part, 'root') and hasattr(part.root, 'text')):
    ...
    async def safe_task_update(task_updater: TaskUpdater, state: TaskState, 
        ...
        try:
            if not message_id:
                message_id = f"task_update_{uuid.uuid4().hex[:12]}"
                
            message = A2AMessageProtocol.create_agent_message(text, message_id)
    ...
    @staticmethod
    def parse_legacy_response(response_data: Union[Dict, Any]) -> Dict[str, Any]:
        """
        레거시 A2A 응답 파싱 (하위 호환성)
        
        Args:
            response_data: 응답 데이터 (dict 또는 객체)
            
        Returns:
            Dict[str, Any]: 파싱된 응답 정보
        """
        result = {
            "success": False,
            "text": "",
        ...
        try:
            if isinstance(response_data, dict):
                ...
                if "result" in response_data:
                    result["success"] = True
                    result["text"] = str(response_data["result"])
                elif "error" in response_data:
                    result["error"] = response_data["error"]
            else:
                # 객체 응답 처리
                extracted_text = A2AMessageProtocol.extract_response_text(response_data)
                if extracted_text:
                    result["success"] = True
...
def create_user_message(text: str, message_id: Optional[str] = None) -> Message:
    """전역 함수: 사용자 메시지 생성"""
    return A2AMessageProtocol.create_user_message(text, message_id)

def create_agent_message(text: str, message_id: Optional[str] = None) -> Message:
    """전역 함수: 에이전트 메시지 생성"""
    return A2AMessageProtocol.create_agent_message(text, message_id)

def extract_user_input(context: RequestContext) -> str:
    """전역 함수: 사용자 입력 추출"""
    return A2AMessageProtocol.extract_user_input(context)

def extract_response_text(response: Any) -> str:
    """전역 함수: 응답 텍스트 추출"""
    return A2AMessageProtocol.extract_response_text(response)

async def safe_task_update(task_updater: TaskUpdater, state: TaskState, 
                          text: str, message_id: Optional[str] = None) -> bool:
    """전역 함수: 안전한 TaskUpdater 메시지 전송"""
    return await A2AMessageProtocol.safe_task_update(task_updater, state, text, message_id) 