...
AI_DS_Team의 FeatureEngineeringAgent를 A2A 프로토콜로 래핑하여 제공합니다.
데이터 시각화 및 차트 생성 전문
"""
...
"""
    ...
    async def cancel(self, context: RequestContext) -> None:
        """작업 취소"""
        logger.info(f"FeatureEngineeringAgent task cancelled: {context.task_id}")
...
def main():
    ...
    agent_card = AgentCard(
        name="AI_DS_Team FeatureEngineeringAgent",
        description="전문적인 데이터 시각화 및 차트 생성 서비스. Plotly, Matplotlib을 활용하여 인터랙티브 및 정적 차트를 생성합니다.",
    ...
    print("📊 Starting AI_DS_Team FeatureEngineeringAgent Server")
