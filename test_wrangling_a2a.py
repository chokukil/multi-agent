#!/usr/bin/env python3
"""
Data Wrangling Agent A2A 공식 테스트
"""
import asyncio
import logging
...
async def test_wrangling_agent():
    """Data Wrangling Agent 테스트"""
    ...
    async with httpx.AsyncClient(timeout=60) as httpx_client:
        try:
            ...
            # Agent Card 가져오기
            logger.info(f'Data Wrangling Agent 카드 조회: {base_url}/.well-known/agent.json')
            public_card = await resolver.get_agent_card()
            logger.info('✅ Data Wrangling Agent 카드 조회 성공:')
            ...
            test_scenarios = [
                {
                    'name': '데이터 필터링 기능',
                    'message': '샘플 데이터에서 특정 조건으로 필터링해주세요'
                },
                {
                    'name': '데이터 정렬 기능', 
                    'message': '데이터를 특정 컬럼 기준으로 정렬해주세요'
                },
                {
                    'name': '데이터 그룹화 및 집계',
                    'message': '데이터를 그룹화하고 평균값을 계산해주세요'
                }
            ]
            ...
            print("\n" + "="*80)
            print("📊 Data Wrangling Agent 테스트 결과")
            print("="*80)
            ...
            logger.error(f'❌ Data Wrangling Agent 테스트 전체 실패: {e}')
            return {
                'agent_name': 'Data Wrangling Agent',
                'total_tests': 0,
                'successful_tests': 0,
                'success_rate': 0,
                'error': str(e)
...
if __name__ == '__main__':
    result = asyncio.run(test_wrangling_agent())
    
    if result['success_rate'] >= 80:
        print(f"\n🎉 Data Wrangling Agent 테스트 성공! ({result['success_rate']:.1f}%)")
    else:
        print(f"\n📋 Data Wrangling Agent 테스트 완료 ({result['success_rate']:.1f}%)")