...
class ArtifactManager:
    ...
    def __init__(self, max_memory_mb: int = 100, max_cache_items: int = 10):
        ...
        logging.info(f"🎨 Artifact Manager initialized: {max_memory_mb}MB limit, {max_cache_items} cache items, {len(self.metadata)} existing artifacts")
    ...
    def create_artifact(self, content: str, artifact_type: str, title: Optional[str] = None, 
                       ...
                       session_id: Optional[str] = None, thread_id: Optional[str] = None) -> str:
        """새 아티팩트 생성"""
        ...
        logging.info(f"🎨 Created artifact {artifact_id}: {title} ({artifact_type}) for session {session_id}")
        return artifact_id
    ...
    def clear_all_artifacts(self) -> bool:
        ...
        try:
            ...
            logging.info("🧹 All artifacts cleared for new session")
            return True
        except Exception as e:
            logging.error(f"❌ Failed to clear artifacts: {e}")
            return False
