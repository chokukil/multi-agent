...
class PlanningEngine:
    ...
    def __init__(self):
        # 하드코딩 제거: LLM 기반 동적 도메인 키워드 생성으로 대체
        self.domain_keywords = {}  # 빈 딕셔너리로 초기화
    ...
    def _extract_required_capabilities(self, analysis_types: List[str], domain: Optional[str]) -> List[str]:
        ...
        # 도메인 특화 능력 - LLM 기반 동적 감지로 대체
        # 하드코딩된 도메인 분기 제거: Universal Engine이 동적으로 도메인별 능력 결정
    ...
    def _get_essential_agents(self, intent: UserIntent, available_agents: List[AgentConfig]) -> List[AgentSelection]:
        ...
        # 동적 에이전트 선택 - 하드코딩된 에이전트 ID 제거
        # Universal Engine이 동적으로 필수 에이전트를 결정
        for agent in available_agents:
            # 데이터 로딩 관련 에이전트 동적 감지
            if any(cap in agent.capabilities for cap in ['data_loading', 'file_processing', 'data_import']):
                essential.append(AgentSelection(
                    agent_id=agent.id,
                    confidence=1.0,
                    reasoning="데이터 로딩 능력을 보유한 에이전트",
        ...
        if 'eda' in intent.analysis_type:
            for agent in available_agents:
                if any(cap in agent.capabilities for cap in ['statistical_analysis', 'data_profiling', 'exploratory_analysis']):
                    essential.append(AgentSelection(
                        agent_id=agent.id,
                        confidence=0.9,
                        reasoning="탐색적 데이터 분석 능력을 보유한 에이전트",
    ...
    def _order_agents_by_dependencies(self, agents: List[AgentSelection], intent: UserIntent) -> List[AgentSelection]:
        ...
        # 동적 에이전트 순서 결정 - 하드코딩된 에이전트 ID 제거
        # 데이터 로딩 관련 에이전트를 첫 번째로
        data_loader = next((agent for agent in remaining 
                          if any(cap in ['data_loading', 'file_processing', 'data_import'] 
                                for cap in getattr(agent, 'capabilities', []))), None)
        if data_loader:
            ordered.append(data_loader)
            remaining.remove(data_loader)
        
        # EDA 관련 에이전트를 두 번째로
        eda_agent = next((agent for agent in remaining 
                        if any(cap in ['statistical_analysis', 'data_profiling', 'exploratory_analysis'] 
                              for cap in getattr(agent, 'capabilities', []))), None)
        if eda_agent:
    ...
    async def _get_domain_keywords_dynamically(self, data: Any, user_query: str) -> Dict[str, List[str]]:
        """LLM 기반 동적 도메인 키워드 생성"""
        try:
            if self.llm_client is None:
                from core.llm_factory import create_llm_instance
                self.llm_client = create_llm_instance()
            
            prompt = f"""
            Based on the data characteristics and user query, generate domain-specific keywords for analysis planning.
            
            Data sample: {str(data)[:300] if data else "No data"}
            User query: {user_query}
            
            Generate keywords for potential domains in JSON format:
            {{
                "detected_domain": ["keyword1", "keyword2", "keyword3"],
                "secondary_domains": {{
                    "domain_name": ["keyword1", "keyword2"]
                }}
            }}
            
            Focus on technical terms, industry-specific vocabulary, and analysis-relevant keywords.
            """
            ...
            # 폴백: 빈 키워드 딕셔너리 반환
            return {"general": ["data", "analysis", "insight"]}