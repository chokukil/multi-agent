#!/usr/bin/env python3
"""
Data Visualization Agent A2A 공식 테스트
"""
import asyncio
import logging
...
async def test_visualization_agent():
    ...
    async with httpx.AsyncClient(timeout=60) as httpx_client:
        try:
            ...
            # Agent Card 가져오기
            logger.info(f'Data Visualization Agent 카드 조회: {base_url}/.well-known/agent.json')
            public_card = await resolver.get_agent_card()
            logger.info('✅ Data Visualization Agent 카드 조회 성공:')
            ...
            test_scenarios = [
                {
                    'name': '기본 차트 생성 기능',
                    'message': '샘플 데이터로 막대 차트를 생성해주세요'
                },
                {
                    'name': '산점도 차트 생성',
                    'message': '두 변수 간 상관관계를 보여주는 산점도를 만들어주세요'
                },
                {
                    'name': '히스토그램 생성',
                    'message': '데이터 분포를 보여주는 히스토그램을 그려주세요'
                }
            ]
            ...
            print("\n" + "="*80)
            print("📊 Data Visualization Agent 테스트 결과")
            print("="*80)
            ...
            logger.error(f'❌ Data Visualization Agent 테스트 전체 실패: {e}')
            return {
...
if __name__ == '__main__':
    result = asyncio.run(test_visualization_agent())
    
    if result['success_rate'] >= 80:
        print(f"\n🎉 Data Visualization Agent 테스트 성공! ({result['success_rate']:.1f}%)")
    else:
        print(f"\n📋 Data Visualization Agent 테스트 완료 ({result['success_rate']:.1f}%)")