#!/usr/bin/env python3
"""
MLflow Agent Fixed 테스트
성공한 패턴을 적용한 MLflow 서버(8323) 검증
"""
...
async def test_mlflow_fixed():
    print("🔬 MLflow Agent Fixed 테스트 시작")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as httpx_client:
            ...
            query = "MLflow 실험 추적 기능을 테스트해주세요"
                ...
                if hasattr(response.result, 'status') and response.result.status:
                    ...
                    if hasattr(status, 'message') and status.message:
                        if hasattr(status.message, 'parts') and status.message.parts:
                            ...
                            # 결과 평가
                            success_indicators = [
                                "mlflow" in response_text.lower(),
                                "실험" in response_text or "experiment" in response_text.lower(),
                                len(response_text) > 100,
                                "추적" in response_text or "tracking" in response_text.lower()
                            ]
                            ...
                            if success_count >= 3:
                                print("🎉 **MLflow Agent Fixed 정상 작동 확인!**")
                                print("✅ **성공한 패턴 적용 성공!**")
                                return True
                            else:
...
async def main():
    print("🔬 MLflow Agent Fixed 검증 시작")
    success = await test_mlflow_fixed()
    if success:
        print("\n✅ **성공한 패턴 적용 테스트 통과!**")
